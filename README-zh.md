...
| `Q` `W` | 可移动固体画笔：柠檬块 / 冰块 —— 单击丢入一个 |
| `9` | 绘制可移动固体（拖拽画出形状，如酒瓶），显示为棕色 |
| `C` | 切换拖拽模式：左键拖动可移动固体，滚轮旋转 |
| 滚轮 | 增减画笔半径（拖拽模式下=旋转抓住的固体） |
| `P` | 清空速度场（水立即停滞） |
| `V` | 清空压力场 |
...
**可移动固体**：`9` 画容器/酒瓶，`Q`/`W` 丢柠檬块/冰块入液。运动学物体（酒瓶，或 `C` 拖拽模式下
抓住的物体）会推动、晃荡液体，倾斜可倒出；动力学物体按密度浮沉（冰浮、柠檬块沉）。见 §6.6。

启动时预置演示（`main.gd` 里 `TEST_SOLIDS`：水池 + 漂浮的冰块 + 下沉的柠檬块 + 来回晃荡的固体条）；
把 `TEST_SOLIDS`/`TEST_MULTIPHASE` 设为 `false` 可回到单块水溃坝起始。
